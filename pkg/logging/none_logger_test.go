...
func TestNoneLogger(t *testing.T) {
	...
	tmpDir, err := os.MkdirTemp("", "nonelogger_test")
	if err != nil {
	...
	t.Run("NoLoggingOccurs", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("Failed to read temp dir after operations: %v", err)
		}

		if len(afterFiles) != len(initialFiles) {
			t.Errorf("Expected no new files, but directory content changed")
		}
